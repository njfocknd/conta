	 ...
	 function select_data($query)
    {
		...
		if($link)
		{
				...
				if ($db_selected)
				{
						...
						if(!$result)
						{					
							//el query esta mal en su sintaxis o tiene algun error
							$result = null;
							$this->error = -1;
							$this->msg = "Error al ejecutar el select: ". mysql_error();
						}
				}  else {
						//no se puede acceder a la bdd
						$result = null;
						$this->error = -2;
						$this->msg = "Error al seleccionar la BD: ". mysql_error();
				}
				// Closing connection
				mysql_close($link);
		}
		else 
		{
				$result = null;
				$this->error = -3;
				$this->msg = "Error en las credenciales de usuario:". mysql_error();
		}
		return $result;
	}

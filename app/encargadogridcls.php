...
$encargado_grid = NULL; // Initialize page object first

class cencargado_grid extends cencargado {
	...
	// Page object name
	var $PageObjName = 'encargado_grid';

	// Grid form hidden field names
	var $FormName = 'fencargadogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["encargado"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idencargado->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_idempleado") && $objForm->HasValue("o_idempleado") && $this->idempleado->CurrentValue <> $this->idempleado->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_tabla") && $objForm->HasValue("o_tabla") && $this->tabla->CurrentValue <> $this->tabla->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idencargado->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->tabla->CurrentValue = "caja_chica";
	...
	function LoadFormValues() {
		...
		$this->tabla->setOldValue($objForm->GetValue("o_tabla"));
		...
		if (!$this->idencargado->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
			$this->idencargado->setFormValue($objForm->GetValue("x_idencargado"));
	}
	...
	function RestoreFormValues() {
		...
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
			$this->idencargado->CurrentValue = $this->idencargado->FormValue;
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idencargado->CurrentValue = strval($arKeys[0]); // idencargado
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		$this->estado->ViewCustomAttributes = "";
			...
			$sSqlWrk = "SELECT `idempleado`, `nombre` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `empleado`";
			$sWhereWrk = "";
			ew_AddFilter($sWhereWrk, $sFilterWrk);
			$this->Lookup_Selecting($this->idempleado, $sWhereWrk); // Call Lookup selecting
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			...
			if (trim(strval($this->idreferencia->CurrentValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`idcaja_chica`" . ew_SearchString("=", $this->idreferencia->CurrentValue, EW_DATATYPE_NUMBER, "");
			}
			...
			if ($rswrk) $rswrk->Close();
			array_unshift($arwrk, array("", $Language->Phrase("PleaseSelect"), "", "", "", "", "", "", ""));
			$this->idreferencia->EditValue = $arwrk;
			}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// tabla
			$this->tabla->SetDbValueDef($rsnew, $this->tabla->CurrentValue, "", $this->tabla->ReadOnly);

			// idreferencia
			$this->idreferencia->SetDbValueDef($rsnew, $this->idreferencia->CurrentValue, 0, $this->idreferencia->ReadOnly);
	...
	function AddRow($rsold = NULL) {
		...
		// tabla
		$this->tabla->SetDbValueDef($rsnew, $this->tabla->CurrentValue, "", strval($this->tabla->CurrentValue) == "");
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->idencargado->setDbValue($conn->Insert_ID());
				$rsnew['idencargado'] = $this->idencargado->DbValue;
			}
		} else {
			if ($this->getSuccessMessage() <> "" || $this->getFailureMessage() <> "") {

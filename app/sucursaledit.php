...
fsucursaledit.Lists["x_idpais"] = {"LinkField":"x_idpais","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<select data-field="x_casa_matriz" id="x_casa_matriz" name="x_casa_matriz"<?php echo $sucursal->casa_matriz->EditAttributes() ?>>
<?php
...
<select data-field="x_idpais" id="x_idpais" name="x_idpais"<?php echo $sucursal->idpais->EditAttributes() ?>>
<?php
...
<select data-field="x_estado" id="x_estado" name="x_estado"<?php echo $sucursal->estado->EditAttributes() ?>>
<?php
if (is_array($sucursal->estado->EditValue)) {
	...
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($sucursal->estado->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>

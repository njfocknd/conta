...
class cpais_view extends cpais {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["idpais"] <> "") {
				...
				$this->idpais->setFormValue($_POST["idpais"]);
				$this->RecKey["idpais"] = $this->idpais->FormValue;
			} else {
	...
	function SetupOtherOptions() {
		...
		// "detail_persona"
		$item = &$option->Add("detail_persona");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("persona", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["persona_grid"] && $GLOBALS["persona_grid"]->DetailView) {
		...
		if ($GLOBALS["persona_grid"] && $GLOBALS["persona_grid"]->DetailEdit) {
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("persona", $DetailTblVar)) {
				if (!isset($GLOBALS["persona_grid"]))
					$GLOBALS["persona_grid"] = new cpersona_grid;
				if ($GLOBALS["persona_grid"]->DetailView) {
					$GLOBALS["persona_grid"]->CurrentMode = "view";

					// Save current master table to detail table
					$GLOBALS["persona_grid"]->setCurrentMasterTable($this->TableVar);
					$GLOBALS["persona_grid"]->setStartRecordNumber(1);
					$GLOBALS["persona_grid"]->idpais->FldIsDetailKey = TRUE;
					$GLOBALS["persona_grid"]->idpais->CurrentValue = $this->idpais->CurrentValue;
					$GLOBALS["persona_grid"]->idpais->setSessionValue($GLOBALS["persona_grid"]->idpais->CurrentValue);
				}
			}
		}
	}
...
var CurrentForm = fpaisview = new ew_Form("fpaisview", "view");
...
fpaisview.Lists["x_estado"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpaisview.Lists["x_estado"].Options = <?php echo json_encode($pais->estado->Options()) ?>;
...
<?php if ($pais->idpais->Visible) { // idpais ?>
	<tr id="r_idpais">
		...
		<td data-name="idpais"<?php echo $pais->idpais->CellAttributes() ?>>
...
<?php if ($pais->fecha_insercion->Visible) { // fecha_insercion ?>
	<tr id="r_fecha_insercion">
		...
		<td data-name="fecha_insercion"<?php echo $pais->fecha_insercion->CellAttributes() ?>>
<span id="el_pais_fecha_insercion">
...
<?php
	if (in_array("persona", explode(",", $pais->getCurrentDetailTable())) && $persona->DetailView) {
?>
<?php if ($pais->getCurrentDetailTable() <> "") { ?>
<h4 class="ewDetailCaption"><?php echo $Language->TablePhrase("persona", "TblCaption") ?></h4>
<?php } ?>
<?php include_once "personagrid.php" ?>
<?php } ?>
</form>
<script type="text/javascript">
fpaisview.Init();

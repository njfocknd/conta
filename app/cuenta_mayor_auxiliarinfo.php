...
class ccuenta_mayor_auxiliar extends cTable {
	...
	function __construct() {
		...
		$this->idcuenta_mayor_auxiliar = new cField('cuenta_mayor_auxiliar', 'cuenta_mayor_auxiliar', 'x_idcuenta_mayor_auxiliar', 'idcuenta_mayor_auxiliar', '`idcuenta_mayor_auxiliar`', '`idcuenta_mayor_auxiliar`', 3, -1, FALSE, '`idcuenta_mayor_auxiliar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// nomenclatura
		$this->nomenclatura = new cField('cuenta_mayor_auxiliar', 'cuenta_mayor_auxiliar', 'x_nomenclatura', 'nomenclatura', '`nomenclatura`', '`nomenclatura`', 200, -1, FALSE, '`nomenclatura`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// estado
		$this->estado = new cField('cuenta_mayor_auxiliar', 'cuenta_mayor_auxiliar', 'x_estado', 'estado', '`estado`', '`estado`', 202, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetMasterFilter() {
		...
		if ($this->getCurrentMasterTable() == "cuenta_mayor_principal") {
			if ($this->idcuenta_mayor_principal->getSessionValue() <> "")
				$sMasterFilter .= "`idcuenta_mayor_principal`=" . ew_QuotedValue($this->idcuenta_mayor_principal->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "cuenta_mayor_principal") {
			if ($this->idcuenta_mayor_principal->getSessionValue() <> "")
				$sDetailFilter .= "`idcuenta_mayor_principal`=" . ew_QuotedValue($this->idcuenta_mayor_principal->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	// Update Table
	var $UpdateTable = "`cuenta_mayor_auxiliar`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('idcuenta_mayor_auxiliar', $rs))
				ew_AddFilter($where, ew_QuotedName('idcuenta_mayor_auxiliar') . '=' . ew_QuotedValue($rs['idcuenta_mayor_auxiliar'], $this->idcuenta_mayor_auxiliar->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@idcuenta_mayor_auxiliar@", ew_AdjustSql($this->idcuenta_mayor_auxiliar->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("cuenta_mayor_auxiliarview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("cuenta_mayor_auxiliarview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "cuenta_mayor_auxiliaradd.php?" . $this->UrlParm($parm);
		else
			return "cuenta_mayor_auxiliaradd.php";
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["idcuenta_mayor_auxiliar"]; // idcuenta_mayor_auxiliar

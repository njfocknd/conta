...
$cuenta_mayor_auxiliar = NULL;

//
// Table class for cuenta_mayor_auxiliar
//
class ccuenta_mayor_auxiliar extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 'cuenta_mayor_auxiliar';
		$this->TableName = 'cuenta_mayor_auxiliar';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`cuenta_mayor_auxiliar`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		$this->idcuenta_mayor_auxiliar = new cField('cuenta_mayor_auxiliar', 'cuenta_mayor_auxiliar', 'x_idcuenta_mayor_auxiliar', 'idcuenta_mayor_auxiliar', '`idcuenta_mayor_auxiliar`', '`idcuenta_mayor_auxiliar`', 3, -1, FALSE, '`idcuenta_mayor_auxiliar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// nomenclatura
		$this->nomenclatura = new cField('cuenta_mayor_auxiliar', 'cuenta_mayor_auxiliar', 'x_nomenclatura', 'nomenclatura', '`nomenclatura`', '`nomenclatura`', 200, -1, FALSE, '`nomenclatura`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['nomenclatura'] = &$this->nomenclatura;

		// nombre
		$this->nombre = new cField('cuenta_mayor_auxiliar', 'cuenta_mayor_auxiliar', 'x_nombre', 'nombre', '`nombre`', '`nombre`', 200, -1, FALSE, '`nombre`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['nombre'] = &$this->nombre;

		// idcuenta_mayor_principal
		$this->idcuenta_mayor_principal = new cField('cuenta_mayor_auxiliar', 'cuenta_mayor_auxiliar', 'x_idcuenta_mayor_principal', 'idcuenta_mayor_principal', '`idcuenta_mayor_principal`', '`idcuenta_mayor_principal`', 3, -1, FALSE, '`idcuenta_mayor_principal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// definicion
		$this->definicion = new cField('cuenta_mayor_auxiliar', 'cuenta_mayor_auxiliar', 'x_definicion', 'definicion', '`definicion`', '`definicion`', 200, -1, FALSE, '`definicion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['definicion'] = &$this->definicion;

		// estado
		$this->estado = new cField('cuenta_mayor_auxiliar', 'cuenta_mayor_auxiliar', 'x_estado', 'estado', '`estado`', '`estado`', 202, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "cuenta_mayor_principal") {
			if ($this->idcuenta_mayor_principal->getSessionValue() <> "")
				$sMasterFilter .= "`idcuenta_mayor_principal`=" . ew_QuotedValue($this->idcuenta_mayor_principal->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "cuenta_mayor_principal") {
			if ($this->idcuenta_mayor_principal->getSessionValue() <> "")
				$sDetailFilter .= "`idcuenta_mayor_principal`=" . ew_QuotedValue($this->idcuenta_mayor_principal->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_cuenta_mayor_principal() {
		return "`idcuenta_mayor_principal`=@idcuenta_mayor_principal@";
	}

	// Detail filter
	function SqlDetailFilter_cuenta_mayor_principal() {
		return "`idcuenta_mayor_principal`=@idcuenta_mayor_principal@";
	}
	...
	function GetDetailUrl() {
		...
		if ($this->getCurrentDetailTable() == "subcuenta") {
			$sDetailUrl = $GLOBALS["subcuenta"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`cuenta_mayor_auxiliar`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('idcuenta_mayor_auxiliar', $rs))
				ew_AddFilter($where, ew_QuotedName('idcuenta_mayor_auxiliar', $this->DBID) . '=' . ew_QuotedValue($rs['idcuenta_mayor_auxiliar'], $this->idcuenta_mayor_auxiliar->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->idcuenta_mayor_auxiliar->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@idcuenta_mayor_auxiliar@", ew_AdjustSql($this->idcuenta_mayor_auxiliar->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "cuenta_mayor_auxiliarlist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cuenta_mayor_auxiliarview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cuenta_mayor_auxiliarview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "cuenta_mayor_auxiliaradd.php?" . $this->UrlParm($parm);
		else
			$url = "cuenta_mayor_auxiliaradd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cuenta_mayor_auxiliaredit.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cuenta_mayor_auxiliaredit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cuenta_mayor_auxiliaradd.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cuenta_mayor_auxiliaradd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("cuenta_mayor_auxiliardelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "cuenta_mayor_principal" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_idcuenta_mayor_principal=" . urlencode($this->idcuenta_mayor_principal->CurrentValue);
		}
		return $url;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "idcuenta_mayor_auxiliar:" . ew_VarToJson($this->idcuenta_mayor_auxiliar->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->idcuenta_mayor_auxiliar->CurrentValue)) {
			$sUrl .= "idcuenta_mayor_auxiliar=" . urlencode($this->idcuenta_mayor_auxiliar->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["idcuenta_mayor_auxiliar"]))
				$arKeys[] = ew_StripSlashes($_POST["idcuenta_mayor_auxiliar"]);
			elseif (isset($_GET["idcuenta_mayor_auxiliar"]))
				$arKeys[] = ew_StripSlashes($_GET["idcuenta_mayor_auxiliar"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->idcuenta_mayor_auxiliar->CurrentValue = $key;

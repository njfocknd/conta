...
class cempleado extends cTable {
	...
	function __construct() {
		...
		// idempleado
		$this->idempleado = new cField('empleado', 'empleado', 'x_idempleado', 'idempleado', '`idempleado`', '`idempleado`', 3, -1, FALSE, '`idempleado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// codigo
		$this->codigo = new cField('empleado', 'empleado', 'x_codigo', 'codigo', '`codigo`', '`codigo`', 200, -1, FALSE, '`codigo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// idpersona
		$this->idpersona = new cField('empleado', 'empleado', 'x_idpersona', 'idpersona', '`idpersona`', '`idpersona`', 3, -1, FALSE, '`idpersona`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->idpersona->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idpersona'] = &$this->idpersona;

		// estado
		$this->estado = new cField('empleado', 'empleado', 'x_estado', 'estado', '`estado`', '`estado`', 202, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->estado->OptionCount = 2;
		$this->fields['estado'] = &$this->estado;

		// idempresa
		$this->idempresa = new cField('empleado', 'empleado', 'x_idempresa', 'idempresa', '`idempresa`', '`idempresa`', 3, -1, FALSE, '`idempresa`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('idempleado', $rs))
				ew_AddFilter($where, ew_QuotedName('idempleado', $this->DBID) . '=' . ew_QuotedValue($rs['idempleado'], $this->idempleado->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@idempleado@", ew_AdjustSql($this->idempleado->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "idempleado:" . ew_VarToJson($this->idempleado->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["idempleado"]))
				$arKeys[] = ew_StripSlashes($_POST["idempleado"]);
			elseif (isset($_GET["idempleado"]))
				$arKeys[] = ew_StripSlashes($_GET["idempleado"]);
			else

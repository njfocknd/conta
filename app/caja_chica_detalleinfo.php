...
class ccaja_chica_detalle extends cTable {
	...
	function __construct() {
		...
		$this->idcaja_chica_detalle = new cField('caja_chica_detalle', 'caja_chica_detalle', 'x_idcaja_chica_detalle', 'idcaja_chica_detalle', '`idcaja_chica_detalle`', '`idcaja_chica_detalle`', 3, -1, FALSE, '`idcaja_chica_detalle`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// idcaja_chica
		$this->idcaja_chica = new cField('caja_chica_detalle', 'caja_chica_detalle', 'x_idcaja_chica', 'idcaja_chica', '`idcaja_chica`', '`idcaja_chica`', 3, -1, FALSE, '`idcaja_chica`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idcaja_chica->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idcaja_chica'] = &$this->idcaja_chica;

		// fecha
		$this->fecha = new cField('caja_chica_detalle', 'caja_chica_detalle', 'x_fecha', 'fecha', '`fecha`', 'DATE_FORMAT(`fecha`, \'%d/%m/%Y\')', 133, 7, FALSE, '`fecha`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fecha->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['fecha'] = &$this->fecha;

		// monto
		$this->monto = new cField('caja_chica_detalle', 'caja_chica_detalle', 'x_monto', 'monto', '`monto`', '`monto`', 131, -1, FALSE, '`monto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->monto->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['monto'] = &$this->monto;

		// tipo
		$this->tipo = new cField('caja_chica_detalle', 'caja_chica_detalle', 'x_tipo', 'tipo', '`tipo`', '`tipo`', 202, -1, FALSE, '`tipo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['tipo'] = &$this->tipo;

		// fecha_insercion
		$this->fecha_insercion = new cField('caja_chica_detalle', 'caja_chica_detalle', 'x_fecha_insercion', 'fecha_insercion', '`fecha_insercion`', 'DATE_FORMAT(`fecha_insercion`, \'%d/%m/%Y\')', 135, 7, FALSE, '`fecha_insercion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// estado
		$this->estado = new cField('caja_chica_detalle', 'caja_chica_detalle', 'x_estado', 'estado', '`estado`', '`estado`', 202, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	// Update Table
	var $UpdateTable = "`caja_chica_detalle`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('idcaja_chica_detalle', $rs))
				ew_AddFilter($where, ew_QuotedName('idcaja_chica_detalle') . '=' . ew_QuotedValue($rs['idcaja_chica_detalle'], $this->idcaja_chica_detalle->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@idcaja_chica_detalle@", ew_AdjustSql($this->idcaja_chica_detalle->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("caja_chica_detalleview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("caja_chica_detalleview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "caja_chica_detalleadd.php?" . $this->UrlParm($parm);
		else
			return "caja_chica_detalleadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("caja_chica_detalleedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("caja_chica_detalleadd.php", $this->UrlParm($parm));
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["idcaja_chica_detalle"]; // idcaja_chica_detalle

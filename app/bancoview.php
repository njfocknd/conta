...
class cbanco_view extends cbanco {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			} elseif (@$_POST["idbanco"] <> "") {
				$this->idbanco->setFormValue($_POST["idbanco"]);
				$this->RecKey["idbanco"] = $this->idbanco->FormValue;
			} else {
	...
	function SetupOtherOptions() {
		...
		// "detail_banco_cuenta"
		$item = &$option->Add("detail_banco_cuenta");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("banco_cuenta", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["banco_cuenta_grid"] && $GLOBALS["banco_cuenta_grid"]->DetailView) {
		...
		if ($GLOBALS["banco_cuenta_grid"] && $GLOBALS["banco_cuenta_grid"]->DetailEdit) {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("banco_cuenta", $DetailTblVar)) {
				...
				if ($GLOBALS["banco_cuenta_grid"]->DetailView) {
					$GLOBALS["banco_cuenta_grid"]->CurrentMode = "view";
...
var CurrentForm = fbancoview = new ew_Form("fbancoview", "view");
...
fbancoview.Lists["x_estado"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fbancoview.Lists["x_estado"].Options = <?php echo json_encode($banco->estado->Options()) ?>;
...
<?php if ($banco->idbanco->Visible) { // idbanco ?>
	<tr id="r_idbanco">
		...
		<td data-name="idbanco"<?php echo $banco->idbanco->CellAttributes() ?>>
<span id="el_banco_idbanco">
...
<?php if ($banco->fecha_insercion->Visible) { // fecha_insercion ?>
	<tr id="r_fecha_insercion">
		...
		<td data-name="fecha_insercion"<?php echo $banco->fecha_insercion->CellAttributes() ?>>
<span id="el_banco_fecha_insercion">
...
<?php if ($banco->estado->Visible) { // estado ?>
	<tr id="r_estado">
		...
		<td data-name="estado"<?php echo $banco->estado->CellAttributes() ?>>
...
<?php
	if (in_array("banco_cuenta", explode(",", $banco->getCurrentDetailTable())) && $banco_cuenta->DetailView) {
?>

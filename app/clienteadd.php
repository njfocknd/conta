...
fclienteadd.Lists["x_idpersona"] = {"LinkField":"x_idpersona","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fclienteadd.Lists["x_idempresa"] = {"LinkField":"x_idempresa","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php if ($cliente->getCurrentMasterTable() == "persona") { ?>
...
<?php if ($cliente->nombre->Visible) { // nombre ?>
	<div id="r_nombre" class="form-group">
		<label id="elh_cliente_nombre" for="x_nombre" class="col-sm-2 control-label ewLabel"><?php echo $cliente->nombre->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $cliente->nombre->CellAttributes() ?>>
<span id="el_cliente_nombre">
<input type="text" data-table="cliente" data-field="x_nombre" name="x_nombre" id="x_nombre" size="30" maxlength="45" placeholder="<?php echo ew_HtmlEncode($cliente->nombre->getPlaceHolder()) ?>" value="<?php echo $cliente->nombre->EditValue ?>"<?php echo $cliente->nombre->EditAttributes() ?>>
</span>
...
<?php if ($cliente->direccion->Visible) { // direccion ?>
	<div id="r_direccion" class="form-group">
		<label id="elh_cliente_direccion" for="x_direccion" class="col-sm-2 control-label ewLabel"><?php echo $cliente->direccion->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $cliente->direccion->CellAttributes() ?>>
<span id="el_cliente_direccion">
<input type="text" data-table="cliente" data-field="x_direccion" name="x_direccion" id="x_direccion" size="30" maxlength="45" placeholder="<?php echo ew_HtmlEncode($cliente->direccion->getPlaceHolder()) ?>" value="<?php echo $cliente->direccion->EditValue ?>"<?php echo $cliente->direccion->EditAttributes() ?>>
</span>
<?php echo $cliente->direccion->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cliente->idpersona->Visible) { // idpersona ?>
	<div id="r_idpersona" class="form-group">
		<label id="elh_cliente_idpersona" for="x_idpersona" class="col-sm-2 control-label ewLabel"><?php echo $cliente->idpersona->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $cliente->idpersona->CellAttributes() ?>>
<?php if ($cliente->idpersona->getSessionValue() <> "") { ?>
<span id="el_cliente_idpersona">
<span<?php echo $cliente->idpersona->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $cliente->idpersona->ViewValue ?></p></span>
</span>
<input type="hidden" id="x_idpersona" name="x_idpersona" value="<?php echo ew_HtmlEncode($cliente->idpersona->CurrentValue) ?>">
<?php } else { ?>
<span id="el_cliente_idpersona">
<select data-table="cliente" data-field="x_idpersona" data-value-separator="<?php echo ew_HtmlEncode(is_array($cliente->idpersona->DisplayValueSeparator) ? json_encode($cliente->idpersona->DisplayValueSeparator) : $cliente->idpersona->DisplayValueSeparator) ?>" id="x_idpersona" name="x_idpersona"<?php echo $cliente->idpersona->EditAttributes() ?>>
<?php
if (is_array($cliente->idpersona->EditValue)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = ew_SameStr($cliente->idpersona->CurrentValue, $arwrk[$rowcntwrk][0]) ? " selected" : "";
		if ($selwrk <> "") $emptywrk = FALSE;		
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $cliente->idpersona->DisplayValue($arwrk[$rowcntwrk]) ?>
</option>
<?php
	}
	if ($emptywrk && strval($cliente->idpersona->CurrentValue) <> "") {
?>
<option value="<?php echo ew_HtmlEncode($cliente->idpersona->CurrentValue) ?>" selected><?php echo $cliente->idpersona->CurrentValue ?></option>
<?php
    }
}
?>
...
$lookuptblfilter = "`estado` = 'Activo'";
ew_AddFilter($sWhereWrk, $lookuptblfilter);
$cliente->idpersona->LookupFilters = array("s" => $sSqlWrk, "d" => "");
$cliente->idpersona->LookupFilters += array("f0" => "`idpersona` = {filter_value}", "t0" => "3", "fn0" => "");
$sSqlWrk = "";
$cliente->Lookup_Selecting($cliente->idpersona, $sWhereWrk); // Call Lookup selecting
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
$sSqlWrk .= " ORDER BY `nombre`";
if ($sSqlWrk <> "") $cliente->idpersona->LookupFilters["s"] .= $sSqlWrk;
?>
<input type="hidden" name="s_x_idpersona" id="s_x_idpersona" value="<?php echo $cliente->idpersona->LookupFilterQuery() ?>">
</span>
<?php } ?>
<?php echo $cliente->idpersona->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($cliente->idempresa->Visible) { // idempresa ?>
	<div id="r_idempresa" class="form-group">
		<label id="elh_cliente_idempresa" for="x_idempresa" class="col-sm-2 control-label ewLabel"><?php echo $cliente->idempresa->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $cliente->idempresa->CellAttributes() ?>>
<span id="el_cliente_idempresa">
<select data-table="cliente" data-field="x_idempresa" data-value-separator="<?php echo ew_HtmlEncode(is_array($cliente->idempresa->DisplayValueSeparator) ? json_encode($cliente->idempresa->DisplayValueSeparator) : $cliente->idempresa->DisplayValueSeparator) ?>" id="x_idempresa" name="x_idempresa"<?php echo $cliente->idempresa->EditAttributes() ?>>
<?php
...
?>
<input type="hidden" name="s_x_idempresa" id="s_x_idempresa" value="<?php echo $cliente->idempresa->LookupFilterQuery() ?>">
</span>
<?php echo $cliente->idempresa->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>

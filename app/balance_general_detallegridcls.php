...
$balance_general_detalle_grid = NULL; // Initialize page object first

class cbalance_general_detalle_grid extends cbalance_general_detalle {
	...
	// Page object name
	var $PageObjName = 'balance_general_detalle_grid';

	// Grid form hidden field names
	var $FormName = 'fbalance_general_detallegrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["balance_general_detalle"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('idbalance_general_detalle');
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idbalance_general_detalle->CurrentValue = strval($arKeys[0]); // idbalance_general_detalle
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "balance_general") {
			$this->idbalance_general->Visible = FALSE;
			if ($GLOBALS["balance_general"]->EventCancelled) $this->EventCancelled = TRUE;
		}

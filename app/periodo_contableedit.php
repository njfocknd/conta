...
var CurrentForm = fperiodo_contableedit = new ew_Form("fperiodo_contableedit", "edit");
...
fperiodo_contableedit.Lists["x_estado"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fperiodo_contableedit.Lists["x_estado"].Options = <?php echo json_encode($periodo_contable->estado->Options()) ?>;
fperiodo_contableedit.Lists["x_estatus"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fperiodo_contableedit.Lists["x_estatus"].Options = <?php echo json_encode($periodo_contable->estatus->Options()) ?>;
...
?>
<form name="fperiodo_contableedit" id="fperiodo_contableedit" class="<?php echo $periodo_contable_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<script type="text/javascript">
ew_CreateCalendar("fperiodo_contableedit", "x_fecha_inicio", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fperiodo_contableedit", "x_fecha_fin", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<?php if ($periodo_contable->estado->Visible) { // estado ?>
	<div id="r_estado" class="form-group">
		<label id="elh_periodo_contable_estado" for="x_estado" class="col-sm-2 control-label ewLabel"><?php echo $periodo_contable->estado->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $periodo_contable->estado->CellAttributes() ?>>
<span id="el_periodo_contable_estado">
<select data-table="periodo_contable" data-field="x_estado" data-value-separator="<?php echo ew_HtmlEncode(is_array($periodo_contable->estado->DisplayValueSeparator) ? json_encode($periodo_contable->estado->DisplayValueSeparator) : $periodo_contable->estado->DisplayValueSeparator) ?>" id="x_estado" name="x_estado"<?php echo $periodo_contable->estado->EditAttributes() ?>>
<?php
if (is_array($periodo_contable->estado->EditValue)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = ew_SameStr($periodo_contable->estado->CurrentValue, $arwrk[$rowcntwrk][0]) ? " selected" : "";
		if ($selwrk <> "") $emptywrk = FALSE;		
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $periodo_contable->estado->DisplayValue($arwrk[$rowcntwrk]) ?>
</option>
<?php
	}
	if ($emptywrk && strval($periodo_contable->estado->CurrentValue) <> "") {
?>
<option value="<?php echo ew_HtmlEncode($periodo_contable->estado->CurrentValue) ?>" selected><?php echo $periodo_contable->estado->CurrentValue ?></option>
<?php
    }
}
?>
...
<input type="hidden" data-table="periodo_contable" data-field="x_idperiodo_contable" name="x_idperiodo_contable" id="x_idperiodo_contable" value="<?php echo ew_HtmlEncode($periodo_contable->idperiodo_contable->CurrentValue) ?>">
<?php
	if (in_array("balance_general", explode(",", $periodo_contable->getCurrentDetailTable())) && $balance_general->DetailEdit) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $periodo_contable_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">

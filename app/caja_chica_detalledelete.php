...
$caja_chica_detalle_delete = NULL; // Initialize page object first

class ccaja_chica_detalle_delete extends ccaja_chica_detalle {
	...
	// Page object name
	var $PageObjName = 'caja_chica_detalle_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("caja_chica_detallelist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in caja_chica_detalle class, caja_chica_detalleinfo.php
...
// Create page object
if (!isset($caja_chica_detalle_delete)) $caja_chica_detalle_delete = new ccaja_chica_detalle_delete();

// Page init
$caja_chica_detalle_delete->Page_Init();

// Page main
$caja_chica_detalle_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$caja_chica_detalle_delete->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var caja_chica_detalle_delete = new ew_Page("caja_chica_detalle_delete");
caja_chica_detalle_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = caja_chica_detalle_delete.PageID; // For backward compatibility

// Form object
var fcaja_chica_detalledelete = new ew_Form("fcaja_chica_detalledelete");

// Form_CustomValidate event
fcaja_chica_detalledelete.Form_CustomValidate = 
...
fcaja_chica_detalledelete.ValidateRequired = true;
<?php } else { ?>
fcaja_chica_detalledelete.ValidateRequired = false; 
<?php } ?>
...
if ($caja_chica_detalle_delete->Recordset = $caja_chica_detalle_delete->LoadRecordset())
	$caja_chica_detalle_deleteTotalRecs = $caja_chica_detalle_delete->Recordset->RecordCount(); // Get record count
if ($caja_chica_detalle_deleteTotalRecs <= 0) { // No record found, exit
	if ($caja_chica_detalle_delete->Recordset)
		$caja_chica_detalle_delete->Recordset->Close();
	$caja_chica_detalle_delete->Page_Terminate("caja_chica_detallelist.php"); // Return to list
}
?>
<div class="ewToolbar">
...
<?php $caja_chica_detalle_delete->ShowPageHeader(); ?>
<?php
$caja_chica_detalle_delete->ShowMessage();
?>
<form name="fcaja_chica_detalledelete" id="fcaja_chica_detalledelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($caja_chica_detalle_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $caja_chica_detalle_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="caja_chica_detalle">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($caja_chica_detalle_delete->RecKeys as $key) { ?>
...
<?php if ($caja_chica_detalle->fecha->Visible) { // fecha ?>
		<th><span id="elh_caja_chica_detalle_fecha" class="caja_chica_detalle_fecha"><?php echo $caja_chica_detalle->fecha->FldCaption() ?></span></th>
<?php } ?>
<?php if ($caja_chica_detalle->monto->Visible) { // monto ?>
		<th><span id="elh_caja_chica_detalle_monto" class="caja_chica_detalle_monto"><?php echo $caja_chica_detalle->monto->FldCaption() ?></span></th>
<?php } ?>
<?php if ($caja_chica_detalle->tipo->Visible) { // tipo ?>
		<th><span id="elh_caja_chica_detalle_tipo" class="caja_chica_detalle_tipo"><?php echo $caja_chica_detalle->tipo->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$caja_chica_detalle_delete->RecCnt = 0;
$i = 0;
while (!$caja_chica_detalle_delete->Recordset->EOF) {
	$caja_chica_detalle_delete->RecCnt++;
	$caja_chica_detalle_delete->RowCnt++;

	// Set row properties
	$caja_chica_detalle->ResetAttrs();
	$caja_chica_detalle->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$caja_chica_detalle_delete->LoadRowValues($caja_chica_detalle_delete->Recordset);

	// Render row
	$caja_chica_detalle_delete->RenderRow();
?>
	<tr<?php echo $caja_chica_detalle->RowAttributes() ?>>
<?php if ($caja_chica_detalle->fecha->Visible) { // fecha ?>
		<td<?php echo $caja_chica_detalle->fecha->CellAttributes() ?>>
<span id="el<?php echo $caja_chica_detalle_delete->RowCnt ?>_caja_chica_detalle_fecha" class="form-group caja_chica_detalle_fecha">
...
<span id="el<?php echo $caja_chica_detalle_delete->RowCnt ?>_caja_chica_detalle_monto" class="form-group caja_chica_detalle_monto">
...
<span id="el<?php echo $caja_chica_detalle_delete->RowCnt ?>_caja_chica_detalle_tipo" class="form-group caja_chica_detalle_tipo">
...
<?php
	$caja_chica_detalle_delete->Recordset->MoveNext();
}
$caja_chica_detalle_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
fcaja_chica_detalledelete.Init();
</script>
<?php
$caja_chica_detalle_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$caja_chica_detalle_delete->Page_Terminate();
?>

...
$sucursal_grid = NULL; // Initialize page object first

class csucursal_grid extends csucursal {
	...
	// Page object name
	var $PageObjName = 'sucursal_grid';

	// Grid form hidden field names
	var $FormName = 'fsucursalgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["sucursal"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idsucursal->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_nombre") && $objForm->HasValue("o_nombre") && $this->nombre->CurrentValue <> $this->nombre->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_casa_matriz") && $objForm->HasValue("o_casa_matriz") && $this->casa_matriz->CurrentValue <> $this->casa_matriz->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idsucursal->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->casa_matriz->CurrentValue = "No";
		$this->casa_matriz->OldValue = $this->casa_matriz->CurrentValue;
	...
	function LoadFormValues() {
		...
		$this->casa_matriz->setOldValue($objForm->GetValue("o_casa_matriz"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idsucursal->CurrentValue = strval($arKeys[0]); // idsucursal
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// casa_matriz
		$this->casa_matriz->SetDbValueDef($rsnew, $this->casa_matriz->CurrentValue, "", strval($this->casa_matriz->CurrentValue) == "");

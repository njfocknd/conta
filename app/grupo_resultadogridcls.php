...
$grupo_resultado_grid = NULL; // Initialize page object first

class cgrupo_resultado_grid extends cgrupo_resultado {
	...
	// Page object name
	var $PageObjName = 'grupo_resultado_grid';

	// Grid form hidden field names
	var $FormName = 'fgrupo_resultadogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["grupo_resultado"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idgrupo_resultado->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idgrupo_resultado->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('idgrupo_resultado');
	}
	...
	function LoadFormValues() {
		...
		if (!$this->idgrupo_resultado->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idgrupo_resultado->CurrentValue = strval($arKeys[0]); // idgrupo_resultado
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['idgrupo_resultado'];
				$conn->raiseErrorFn = $GLOBALS["EW_ERROR_FN"];
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "clase_resultado") {
			$this->idclase_resultado->Visible = FALSE;
			if ($GLOBALS["clase_resultado"]->EventCancelled) $this->EventCancelled = TRUE;
		}

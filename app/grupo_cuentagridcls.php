...
$grupo_cuenta_grid = NULL; // Initialize page object first

class cgrupo_cuenta_grid extends cgrupo_cuenta {
	...
	// Page object name
	var $PageObjName = 'grupo_cuenta_grid';

	// Grid form hidden field names
	var $FormName = 'fgrupo_cuentagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["grupo_cuenta"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idgrupo_cuenta->CurrentValue = strval($arKeys[0]); // idgrupo_cuenta
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "clase_cuenta") {
			$this->idclase_cuenta->Visible = FALSE;
			if ($GLOBALS["clase_cuenta"]->EventCancelled) $this->EventCancelled = TRUE;
		}

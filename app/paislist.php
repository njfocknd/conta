...
<?php include_once "personagridcls.php" ?>
...
class cpais_list extends cpais {
	...
	function __construct() {
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fpaislistsrch";
	...
	function Page_Init() {
		...
		if (@$_POST["ajax"] == "autofill") {
			...
			// Process auto fill for detail table 'persona'
			if (@$_POST["grid"] == "fpersonagrid") {
				if (!isset($GLOBALS["persona_grid"])) $GLOBALS["persona_grid"] = new cpersona_grid;
				$GLOBALS["persona_grid"]->Page_Init();
				$this->Page_Terminate();
				exit();
			}
	...
	function SetupListOptions() {
		...
		// "detail_persona"
		$item = &$this->ListOptions->Add("detail_persona");
		$item->CssStyle = "white-space: nowrap;";
	...
	function RenderListOptions() {
		...
		// "detail_persona"
		$oListOpt = &$this->ListOptions->Items["detail_persona"];
		if (TRUE) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("persona", "TblCaption");
			$body = "<a class=\"btn btn-default btn-sm ewRowLink ewDetail\" data-action=\"list\" href=\"" . ew_HtmlEncode("personalist.php?" . EW_TABLE_SHOW_MASTER . "=pais&fk_idpais=" . urlencode(strval($this->idpais->CurrentValue)) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($GLOBALS["persona_grid"]->DetailView) {
				$links .= "<li><a class=\"ewRowLink ewDetailView\" data-action=\"view\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailViewLink")) . "\" href=\"" . ew_HtmlEncode($this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=persona")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailViewLink")) . "</a></li>";
				if ($DetailViewTblVar <> "") $DetailViewTblVar .= ",";
				$DetailViewTblVar .= "persona";
			}
			if ($GLOBALS["persona_grid"]->DetailEdit) {
				$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailEditLink")) . "\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=persona")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailEditLink")) . "</a></li>";
				if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
				$DetailEditTblVar .= "persona";
			}
			if ($links <> "") {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->idpais->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event);'>";
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_persona");
		$url = $this->GetAddUrl(EW_TABLE_SHOW_DETAIL . "=persona");
		$caption = $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["persona"]->TableCaption();
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($caption) . "\" data-caption=\"" . ew_HtmlTitle($caption) . "\" href=\"" . ew_HtmlEncode($url) . "\">" . $caption . "</a>";
		$item->Visible = ($GLOBALS["persona"]->DetailAdd);
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "persona";
		}
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fpaislistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fpaislistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fpaislist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
...
var CurrentForm = fpaislist = new ew_Form("fpaislist", "list");
...
var CurrentSearchForm = fpaislistsrch = new ew_Form("fpaislistsrch");
</script>
<script type="text/javascript">
...
<?php
	$bSelectLimit = $pais_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($pais_list->TotalRecs <= 0)
			$pais_list->TotalRecs = $pais->SelectRecordCount();
	} else {
		if (!$pais_list->Recordset && ($pais_list->Recordset = $pais_list->LoadRecordset()))
...
// Header row
$pais_list->RowType = EW_ROWTYPE_HEADER;
...
<span id="el<?php echo $pais_list->RowCnt ?>_pais_nombre" class="pais_nombre">

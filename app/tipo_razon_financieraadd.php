...
$tipo_razon_financiera_add = NULL; // Initialize page object first

class ctipo_razon_financiera_add extends ctipo_razon_financiera {
	...
	// Page object name
	var $PageObjName = 'tipo_razon_financiera_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["idtipo_razon_financiera"] != "") {
				...
				$this->setKey("idtipo_razon_financiera", $this->idtipo_razon_financiera->CurrentValue); // Set up key
			} else {
				$this->setKey("idtipo_razon_financiera", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tipo_razon_financieraview.php")
	...
	function LoadDefaultValues() {
		$this->nombre->CurrentValue = "0";
		$this->agrupacion->CurrentValue = " ";
		$this->interpretacion->CurrentValue = " ";
	}
	...
	function AddRow($rsold = NULL) {
		...
		// agrupacion
		$this->agrupacion->SetDbValueDef($rsnew, $this->agrupacion->CurrentValue, "", strval($this->agrupacion->CurrentValue) == "");

		// interpretacion
		$this->interpretacion->SetDbValueDef($rsnew, $this->interpretacion->CurrentValue, NULL, strval($this->interpretacion->CurrentValue) == "");
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->idtipo_razon_financiera->setDbValue($conn->Insert_ID());
				$rsnew['idtipo_razon_financiera'] = $this->idtipo_razon_financiera->DbValue;
			}
		} else {
...
// Create page object
if (!isset($tipo_razon_financiera_add)) $tipo_razon_financiera_add = new ctipo_razon_financiera_add();

// Page init
$tipo_razon_financiera_add->Page_Init();

// Page main
$tipo_razon_financiera_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tipo_razon_financiera_add->Page_Render();
?>
...
var CurrentForm = ftipo_razon_financieraadd = new ew_Form("ftipo_razon_financieraadd", "add");

// Validate form
ftipo_razon_financieraadd.Validate = function() {
...
ftipo_razon_financieraadd.Form_CustomValidate = 
...
ftipo_razon_financieraadd.ValidateRequired = true;
<?php } else { ?>
ftipo_razon_financieraadd.ValidateRequired = false; 
<?php } ?>
...
<?php $tipo_razon_financiera_add->ShowPageHeader(); ?>
<?php
$tipo_razon_financiera_add->ShowMessage();
?>
<form name="ftipo_razon_financieraadd" id="ftipo_razon_financieraadd" class="<?php echo $tipo_razon_financiera_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tipo_razon_financiera_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tipo_razon_financiera_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tipo_razon_financiera_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
ftipo_razon_financieraadd.Init();
</script>
<?php
$tipo_razon_financiera_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tipo_razon_financiera_add->Page_Terminate();
?>

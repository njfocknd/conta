...
$balance_general_grid = NULL; // Initialize page object first

class cbalance_general_grid extends cbalance_general {
	...
	// Page object name
	var $PageObjName = 'balance_general_grid';

	// Grid form hidden field names
	var $FormName = 'fbalance_generalgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["balance_general"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function ClearInlineMode() {
		$this->activo_circulante->FormValue = ""; // Clear form value
		$this->activo_fijo->FormValue = ""; // Clear form value
		$this->pasivo_circulante->FormValue = ""; // Clear form value
		$this->pasivo_fijo->FormValue = ""; // Clear form value
		$this->capital_contable->FormValue = ""; // Clear form value
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idbalance_general->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_idempresa") && $objForm->HasValue("o_idempresa") && $this->idempresa->CurrentValue <> $this->idempresa->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idperioso_contable") && $objForm->HasValue("o_idperioso_contable") && $this->idperioso_contable->CurrentValue <> $this->idperioso_contable->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_activo_circulante") && $objForm->HasValue("o_activo_circulante") && $this->activo_circulante->CurrentValue <> $this->activo_circulante->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_activo_fijo") && $objForm->HasValue("o_activo_fijo") && $this->activo_fijo->CurrentValue <> $this->activo_fijo->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_pasivo_circulante") && $objForm->HasValue("o_pasivo_circulante") && $this->pasivo_circulante->CurrentValue <> $this->pasivo_circulante->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_pasivo_fijo") && $objForm->HasValue("o_pasivo_fijo") && $this->pasivo_fijo->CurrentValue <> $this->pasivo_fijo->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_capital_contable") && $objForm->HasValue("o_capital_contable") && $this->capital_contable->CurrentValue <> $this->capital_contable->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idbalance_general->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('idbalance_general');
	}
	...
	function LoadDefaultValues() {
		...
		$this->idperioso_contable->CurrentValue = 1;
		$this->idperioso_contable->OldValue = $this->idperioso_contable->CurrentValue;
		$this->activo_circulante->CurrentValue = 0.00;
		$this->activo_circulante->OldValue = $this->activo_circulante->CurrentValue;
		$this->activo_fijo->CurrentValue = 0.00;
		$this->activo_fijo->OldValue = $this->activo_fijo->CurrentValue;
		$this->pasivo_circulante->CurrentValue = 0.00;
		$this->pasivo_circulante->OldValue = $this->pasivo_circulante->CurrentValue;
		$this->pasivo_fijo->CurrentValue = 0.00;
		$this->pasivo_fijo->OldValue = $this->pasivo_fijo->CurrentValue;
		$this->capital_contable->CurrentValue = 0.00;
		$this->capital_contable->OldValue = $this->capital_contable->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		if (!$this->idperioso_contable->FldIsDetailKey) {
			$this->idperioso_contable->setFormValue($objForm->GetValue("x_idperioso_contable"));
		}
		$this->idperioso_contable->setOldValue($objForm->GetValue("o_idperioso_contable"));
		if (!$this->activo_circulante->FldIsDetailKey) {
			$this->activo_circulante->setFormValue($objForm->GetValue("x_activo_circulante"));
		}
		$this->activo_circulante->setOldValue($objForm->GetValue("o_activo_circulante"));
		if (!$this->activo_fijo->FldIsDetailKey) {
			$this->activo_fijo->setFormValue($objForm->GetValue("x_activo_fijo"));
		}
		$this->activo_fijo->setOldValue($objForm->GetValue("o_activo_fijo"));
		if (!$this->pasivo_circulante->FldIsDetailKey) {
			$this->pasivo_circulante->setFormValue($objForm->GetValue("x_pasivo_circulante"));
		}
		$this->pasivo_circulante->setOldValue($objForm->GetValue("o_pasivo_circulante"));
		if (!$this->pasivo_fijo->FldIsDetailKey) {
			$this->pasivo_fijo->setFormValue($objForm->GetValue("x_pasivo_fijo"));
		}
		$this->pasivo_fijo->setOldValue($objForm->GetValue("o_pasivo_fijo"));
		if (!$this->capital_contable->FldIsDetailKey) {
			$this->capital_contable->setFormValue($objForm->GetValue("x_capital_contable"));
		}
		$this->capital_contable->setOldValue($objForm->GetValue("o_capital_contable"));
		if (!$this->idbalance_general->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function RestoreFormValues() {
		...
		$this->idperioso_contable->CurrentValue = $this->idperioso_contable->FormValue;
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idbalance_general->CurrentValue = strval($arKeys[0]); // idbalance_general
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		$this->fecha_insercion->ViewCustomAttributes = "";
			...
			if (trim(strval($this->idperioso_contable->CurrentValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`idperiodo_contable`" . ew_SearchString("=", $this->idperioso_contable->CurrentValue, EW_DATATYPE_NUMBER, "");
			}
			...
			if ($rswrk) $rswrk->Close();
			array_unshift($arwrk, array("", $Language->Phrase("PleaseSelect"), "", "", "", "", "", "", ""));
			$this->idperioso_contable->EditValue = $arwrk;
			}
			...
			if (strval($this->activo_circulante->EditValue) <> "" && is_numeric($this->activo_circulante->EditValue)) {
			$this->activo_circulante->EditValue = ew_FormatNumber($this->activo_circulante->EditValue, -2, -1, -2, 0);
			$this->activo_circulante->OldValue = $this->activo_circulante->EditValue;
			}
			...
			if (strval($this->activo_fijo->EditValue) <> "" && is_numeric($this->activo_fijo->EditValue)) {
			$this->activo_fijo->EditValue = ew_FormatNumber($this->activo_fijo->EditValue, -2, -1, -2, 0);
			$this->activo_fijo->OldValue = $this->activo_fijo->EditValue;
			}
			...
			if (strval($this->pasivo_circulante->EditValue) <> "" && is_numeric($this->pasivo_circulante->EditValue)) {
			$this->pasivo_circulante->EditValue = ew_FormatNumber($this->pasivo_circulante->EditValue, -2, -1, -2, 0);
			$this->pasivo_circulante->OldValue = $this->pasivo_circulante->EditValue;
			}
			...
			if (strval($this->pasivo_fijo->EditValue) <> "" && is_numeric($this->pasivo_fijo->EditValue)) {
			$this->pasivo_fijo->EditValue = ew_FormatNumber($this->pasivo_fijo->EditValue, -2, -1, -2, 0);
			$this->pasivo_fijo->OldValue = $this->pasivo_fijo->EditValue;
			}
			...
			if (strval($this->capital_contable->EditValue) <> "" && is_numeric($this->capital_contable->EditValue)) {
			$this->capital_contable->EditValue = ew_FormatNumber($this->capital_contable->EditValue, -2, -1, -2, 0);
			$this->capital_contable->OldValue = $this->capital_contable->EditValue;
			}
	...
	function ValidateForm() {
		...
		if (!$this->idperioso_contable->FldIsDetailKey && !is_null($this->idperioso_contable->FormValue) && $this->idperioso_contable->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->idperioso_contable->FldCaption(), $this->idperioso_contable->ReqErrMsg));
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// idperioso_contable
			$this->idperioso_contable->SetDbValueDef($rsnew, $this->idperioso_contable->CurrentValue, 0, $this->idperioso_contable->ReadOnly);
	...
	function AddRow($rsold = NULL) {
		...
		// idperioso_contable
		$this->idperioso_contable->SetDbValueDef($rsnew, $this->idperioso_contable->CurrentValue, 0, strval($this->idperioso_contable->CurrentValue) == "");

		// activo_circulante
		$this->activo_circulante->SetDbValueDef($rsnew, $this->activo_circulante->CurrentValue, 0, strval($this->activo_circulante->CurrentValue) == "");

		// activo_fijo
		$this->activo_fijo->SetDbValueDef($rsnew, $this->activo_fijo->CurrentValue, 0, strval($this->activo_fijo->CurrentValue) == "");

		// pasivo_circulante
		$this->pasivo_circulante->SetDbValueDef($rsnew, $this->pasivo_circulante->CurrentValue, 0, strval($this->pasivo_circulante->CurrentValue) == "");

		// pasivo_fijo
		$this->pasivo_fijo->SetDbValueDef($rsnew, $this->pasivo_fijo->CurrentValue, 0, strval($this->pasivo_fijo->CurrentValue) == "");

		// capital_contable
		$this->capital_contable->SetDbValueDef($rsnew, $this->capital_contable->CurrentValue, 0, strval($this->capital_contable->CurrentValue) == "");
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "empresa") {
			...
			if ($GLOBALS["empresa"]->EventCancelled) $this->EventCancelled = TRUE;
		}
		if ($sMasterTblVar == "periodo_contable") {
			$this->idperioso_contable->Visible = FALSE;
			if ($GLOBALS["periodo_contable"]->EventCancelled) $this->EventCancelled = TRUE;
		}

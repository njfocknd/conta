...
$cuenta_mayor_principal_grid = NULL; // Initialize page object first

class ccuenta_mayor_principal_grid extends ccuenta_mayor_principal {
	...
	// Page object name
	var $PageObjName = 'cuenta_mayor_principal_grid';

	// Grid form hidden field names
	var $FormName = 'fcuenta_mayor_principalgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["cuenta_mayor_principal"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('idcuenta_mayor_principal');
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idcuenta_mayor_principal->CurrentValue = strval($arKeys[0]); // idcuenta_mayor_principal
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "subgrupo_cuenta") {
			$this->idsubgrupo_cuenta->Visible = FALSE;
			if ($GLOBALS["subgrupo_cuenta"]->EventCancelled) $this->EventCancelled = TRUE;
		}

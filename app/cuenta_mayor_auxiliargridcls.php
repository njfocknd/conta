...
$cuenta_mayor_auxiliar_grid = NULL; // Initialize page object first

class ccuenta_mayor_auxiliar_grid extends ccuenta_mayor_auxiliar {
	...
	// Page object name
	var $PageObjName = 'cuenta_mayor_auxiliar_grid';

	// Grid form hidden field names
	var $FormName = 'fcuenta_mayor_auxiliargrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["cuenta_mayor_auxiliar"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('idcuenta_mayor_auxiliar');
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idcuenta_mayor_auxiliar->CurrentValue = strval($arKeys[0]); // idcuenta_mayor_auxiliar
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "cuenta_mayor_principal") {
			$this->idcuenta_mayor_principal->Visible = FALSE;
			if ($GLOBALS["cuenta_mayor_principal"]->EventCancelled) $this->EventCancelled = TRUE;
		}

...
$empleado_grid = NULL; // Initialize page object first

class cempleado_grid extends cempleado {
	...
	// Page object name
	var $PageObjName = 'empleado_grid';

	// Grid form hidden field names
	var $FormName = 'fempleadogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["empleado"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idempleado->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idempleado->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->idempleado->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idempleado->CurrentValue = strval($arKeys[0]); // idempleado
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;

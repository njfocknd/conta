...
$subgrupo_cuenta_grid = NULL; // Initialize page object first

class csubgrupo_cuenta_grid extends csubgrupo_cuenta {
	...
	// Page object name
	var $PageObjName = 'subgrupo_cuenta_grid';

	// Grid form hidden field names
	var $FormName = 'fsubgrupo_cuentagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["subgrupo_cuenta"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('idsubgrupo_cuenta');
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idsubgrupo_cuenta->CurrentValue = strval($arKeys[0]); // idsubgrupo_cuenta
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "grupo_cuenta") {
			$this->idgrupo_cuenta->Visible = FALSE;
			if ($GLOBALS["grupo_cuenta"]->EventCancelled) $this->EventCancelled = TRUE;
		}

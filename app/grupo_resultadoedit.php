...
$grupo_resultado_edit = NULL; // Initialize page object first

class cgrupo_resultado_edit extends cgrupo_resultado {
	...
	// Page object name
	var $PageObjName = 'grupo_resultado_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->idgrupo_resultado->CurrentValue == "")
			$this->Page_Terminate("grupo_resultadolist.php"); // Invalid key, return to list
...
// Create page object
if (!isset($grupo_resultado_edit)) $grupo_resultado_edit = new cgrupo_resultado_edit();

// Page init
$grupo_resultado_edit->Page_Init();

// Page main
$grupo_resultado_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$grupo_resultado_edit->Page_Render();
?>
...
var CurrentForm = fgrupo_resultadoedit = new ew_Form("fgrupo_resultadoedit", "edit");

// Validate form
fgrupo_resultadoedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $grupo_resultado->estado->FldCaption(), $grupo_resultado->estado->ReqErrMsg)) ?>");
...
fgrupo_resultadoedit.Form_CustomValidate = 
...
fgrupo_resultadoedit.ValidateRequired = true;
<?php } else { ?>
fgrupo_resultadoedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fgrupo_resultadoedit.Lists["x_estado"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fgrupo_resultadoedit.Lists["x_estado"].Options = <?php echo json_encode($grupo_resultado->estado->Options()) ?>;
...
<?php $grupo_resultado_edit->ShowPageHeader(); ?>
<?php
$grupo_resultado_edit->ShowMessage();
?>
<form name="fgrupo_resultadoedit" id="fgrupo_resultadoedit" class="<?php echo $grupo_resultado_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($grupo_resultado_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $grupo_resultado_edit->Token ?>">
<?php } ?>
...
<?php if ($grupo_resultado->estado->Visible) { // estado ?>
	<div id="r_estado" class="form-group">
		<label id="elh_grupo_resultado_estado" for="x_estado" class="col-sm-2 control-label ewLabel"><?php echo $grupo_resultado->estado->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $grupo_resultado->estado->CellAttributes() ?>>
<span id="el_grupo_resultado_estado">
<select data-table="grupo_resultado" data-field="x_estado" data-value-separator="<?php echo ew_HtmlEncode(is_array($grupo_resultado->estado->DisplayValueSeparator) ? json_encode($grupo_resultado->estado->DisplayValueSeparator) : $grupo_resultado->estado->DisplayValueSeparator) ?>" id="x_estado" name="x_estado"<?php echo $grupo_resultado->estado->EditAttributes() ?>>
<?php
if (is_array($grupo_resultado->estado->EditValue)) {
	$arwrk = $grupo_resultado->estado->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = ew_SameStr($grupo_resultado->estado->CurrentValue, $arwrk[$rowcntwrk][0]) ? " selected" : "";
		if ($selwrk <> "") $emptywrk = FALSE;		
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $grupo_resultado->estado->DisplayValue($arwrk[$rowcntwrk]) ?>
</option>
<?php
	}
	if ($emptywrk && strval($grupo_resultado->estado->CurrentValue) <> "") {
?>
<option value="<?php echo ew_HtmlEncode($grupo_resultado->estado->CurrentValue) ?>" selected><?php echo $grupo_resultado->estado->CurrentValue ?></option>
<?php
    }
}
?>
...
<?php echo $grupo_resultado->estado->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<input type="hidden" data-table="grupo_resultado" data-field="x_idgrupo_resultado" name="x_idgrupo_resultado" id="x_idgrupo_resultado" value="<?php echo ew_HtmlEncode($grupo_resultado->idgrupo_resultado->CurrentValue) ?>">
<div class="form-group">
	<div class="col-sm-offset-2 col-sm-10">
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $grupo_resultado_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fgrupo_resultadoedit.Init();
</script>
<?php
$grupo_resultado_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$grupo_resultado_edit->Page_Terminate();
?>

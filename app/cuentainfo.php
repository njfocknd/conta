...
class ccuenta extends cTable {
	...
	function __construct() {
		...
		// idcuenta
		$this->idcuenta = new cField('cuenta', 'cuenta', 'x_idcuenta', 'idcuenta', '`idcuenta`', '`idcuenta`', 3, -1, FALSE, '`idcuenta`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->idcuenta->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idcuenta'] = &$this->idcuenta;

		// nomenclatura
		$this->nomenclatura = new cField('cuenta', 'cuenta', 'x_nomenclatura', 'nomenclatura', '`nomenclatura`', '`nomenclatura`', 200, -1, FALSE, '`nomenclatura`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['nomenclatura'] = &$this->nomenclatura;

		// nombre
		$this->nombre = new cField('cuenta', 'cuenta', 'x_nombre', 'nombre', '`nombre`', '`nombre`', 200, -1, FALSE, '`nombre`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['nombre'] = &$this->nombre;

		// debe
		$this->debe = new cField('cuenta', 'cuenta', 'x_debe', 'debe', '`debe`', '`debe`', 131, -1, FALSE, '`debe`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->debe->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['debe'] = &$this->debe;

		// haber
		$this->haber = new cField('cuenta', 'cuenta', 'x_haber', 'haber', '`haber`', '`haber`', 131, -1, FALSE, '`haber`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->haber->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['haber'] = &$this->haber;

		// saldo
		$this->saldo = new cField('cuenta', 'cuenta', 'x_saldo', 'saldo', '`saldo`', '`saldo`', 131, -1, FALSE, '`saldo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->saldo->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['saldo'] = &$this->saldo;

		// estado
		$this->estado = new cField('cuenta', 'cuenta', 'x_estado', 'estado', '`estado`', '`estado`', 202, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->estado->OptionCount = 2;
		$this->fields['estado'] = &$this->estado;

		// idsubcuenta
		$this->idsubcuenta = new cField('cuenta', 'cuenta', 'x_idsubcuenta', 'idsubcuenta', '`idsubcuenta`', '`idsubcuenta`', 3, -1, FALSE, '`idsubcuenta`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "subcuenta") {
			if ($this->idsubcuenta->getSessionValue() <> "")
				$sMasterFilter .= "`idsubcuenta`=" . ew_QuotedValue($this->idsubcuenta->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "subcuenta") {
			if ($this->idsubcuenta->getSessionValue() <> "")
				$sDetailFilter .= "`idsubcuenta`=" . ew_QuotedValue($this->idsubcuenta->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_subcuenta() {
		return "`idsubcuenta`=@idsubcuenta@";
	}

	// Detail filter
	function SqlDetailFilter_subcuenta() {
		return "`idsubcuenta`=@idsubcuenta@";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('idcuenta', $rs))
				ew_AddFilter($where, ew_QuotedName('idcuenta', $this->DBID) . '=' . ew_QuotedValue($rs['idcuenta'], $this->idcuenta->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@idcuenta@", ew_AdjustSql($this->idcuenta->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cuentaview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cuentaview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "cuentaadd.php?" . $this->UrlParm($parm);
		else
			$url = "cuentaadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("cuentaedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("cuentaadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "subcuenta" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_idsubcuenta=" . urlencode($this->idsubcuenta->CurrentValue);
		}
		return $url;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "idcuenta:" . ew_VarToJson($this->idcuenta->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["idcuenta"]))
				$arKeys[] = ew_StripSlashes($_POST["idcuenta"]);
			elseif (isset($_GET["idcuenta"]))
				$arKeys[] = ew_StripSlashes($_GET["idcuenta"]);
			else
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		if ($this->idsubcuenta->getSessionValue() <> "") {
			$this->idsubcuenta->CurrentValue = $this->idsubcuenta->getSessionValue();
		if (strval($this->idsubcuenta->CurrentValue) <> "") {
			$sFilterWrk = "`idsubcuenta`" . ew_SearchString("=", $this->idsubcuenta->CurrentValue, EW_DATATYPE_NUMBER, "");
		$sSqlWrk = "SELECT `idsubcuenta`, `nombre` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `subcuenta`";
		$sWhereWrk = "";
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->debe->Exportable) $Doc->ExportCaption($this->debe);
					if ($this->haber->Exportable) $Doc->ExportCaption($this->haber);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->debe->Exportable) $Doc->ExportField($this->debe);
						if ($this->haber->Exportable) $Doc->ExportField($this->haber);

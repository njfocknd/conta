...
$grupo_cuenta_view = NULL; // Initialize page object first

class cgrupo_cuenta_view extends cgrupo_cuenta {
	...
	// Page object name
	var $PageObjName = 'grupo_cuenta_view';
	...
	function __construct() {
		...
		if (@$_GET["idgrupo_cuenta"] <> "") {
			$this->RecKey["idgrupo_cuenta"] = $_GET["idgrupo_cuenta"];
			$KeyUrl .= "&amp;idgrupo_cuenta=" . urlencode($this->RecKey["idgrupo_cuenta"]);
		}
	...
	function Page_Init() {
		...
		$this->idgrupo_cuenta->Visible = !$this->IsAdd() && !$this->IsCopy() && !$this->IsGridAdd();
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["idgrupo_cuenta"] <> "") {
				...
				$this->RecKey["idgrupo_cuenta"] = $this->idgrupo_cuenta->QueryStringValue;
			} elseif (@$_POST["idgrupo_cuenta"] <> "") {
				$this->idgrupo_cuenta->setFormValue($_POST["idgrupo_cuenta"]);
				$this->RecKey["idgrupo_cuenta"] = $this->idgrupo_cuenta->FormValue;
			} else {
				$sReturnUrl = "grupo_cuentalist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "grupo_cuentalist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "grupo_cuentalist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		$item = &$option->Add("detail_subgrupo_cuenta");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("subgrupo_cuenta", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["subgrupo_cuenta_grid"] && $GLOBALS["subgrupo_cuenta_grid"]->DetailView) {
		...
		if ($GLOBALS["subgrupo_cuenta_grid"] && $GLOBALS["subgrupo_cuenta_grid"]->DetailEdit) {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("subgrupo_cuenta", $DetailTblVar)) {
				...
				if ($GLOBALS["subgrupo_cuenta_grid"]->DetailView) {
					$GLOBALS["subgrupo_cuenta_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($grupo_cuenta_view)) $grupo_cuenta_view = new cgrupo_cuenta_view();

// Page init
$grupo_cuenta_view->Page_Init();

// Page main
$grupo_cuenta_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$grupo_cuenta_view->Page_Render();
?>
...
var CurrentForm = fgrupo_cuentaview = new ew_Form("fgrupo_cuentaview", "view");

// Form_CustomValidate event
fgrupo_cuentaview.Form_CustomValidate = 
...
fgrupo_cuentaview.ValidateRequired = true;
<?php } else { ?>
fgrupo_cuentaview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fgrupo_cuentaview.Lists["x_idclase_cuenta"] = {"LinkField":"x_idclase_cuenta","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fgrupo_cuentaview.Lists["x_estado"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fgrupo_cuentaview.Lists["x_estado"].Options = <?php echo json_encode($grupo_cuenta->estado->Options()) ?>;
...
<?php $grupo_cuenta_view->ExportOptions->Render("body") ?>
<?php
	foreach ($grupo_cuenta_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $grupo_cuenta_view->ShowPageHeader(); ?>
<?php
$grupo_cuenta_view->ShowMessage();
?>
<form name="fgrupo_cuentaview" id="fgrupo_cuentaview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($grupo_cuenta_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $grupo_cuenta_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="grupo_cuenta">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($grupo_cuenta->idgrupo_cuenta->Visible) { // idgrupo_cuenta ?>
	<tr id="r_idgrupo_cuenta">
		<td><span id="elh_grupo_cuenta_idgrupo_cuenta"><?php echo $grupo_cuenta->idgrupo_cuenta->FldCaption() ?></span></td>
		<td data-name="idgrupo_cuenta"<?php echo $grupo_cuenta->idgrupo_cuenta->CellAttributes() ?>>
<span id="el_grupo_cuenta_idgrupo_cuenta">
<span<?php echo $grupo_cuenta->idgrupo_cuenta->ViewAttributes() ?>>
<?php echo $grupo_cuenta->idgrupo_cuenta->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($grupo_cuenta->nombre->Visible) { // nombre ?>
	<tr id="r_nombre">
		<td><span id="elh_grupo_cuenta_nombre"><?php echo $grupo_cuenta->nombre->FldCaption() ?></span></td>
...
<?php if ($grupo_cuenta->definicion->Visible) { // definicion ?>
	<tr id="r_definicion">
		<td><span id="elh_grupo_cuenta_definicion"><?php echo $grupo_cuenta->definicion->FldCaption() ?></span></td>
		<td data-name="definicion"<?php echo $grupo_cuenta->definicion->CellAttributes() ?>>
<span id="el_grupo_cuenta_definicion">
<span<?php echo $grupo_cuenta->definicion->ViewAttributes() ?>>
<?php echo $grupo_cuenta->definicion->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($grupo_cuenta->estado->Visible) { // estado ?>
	<tr id="r_estado">
		<td><span id="elh_grupo_cuenta_estado"><?php echo $grupo_cuenta->estado->FldCaption() ?></span></td>
...
<?php echo $grupo_cuenta->estado->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php
	if (in_array("subgrupo_cuenta", explode(",", $grupo_cuenta->getCurrentDetailTable())) && $subgrupo_cuenta->DetailView) {
?>
...
<script type="text/javascript">
fgrupo_cuentaview.Init();
</script>
<?php
$grupo_cuenta_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$grupo_cuenta_view->Page_Terminate();
?>

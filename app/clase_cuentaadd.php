...
$clase_cuenta_add = NULL; // Initialize page object first

class cclase_cuenta_add extends cclase_cuenta {
	...
	// Page object name
	var $PageObjName = 'clase_cuenta_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["idclase_cuenta"] != "") {
				...
				$this->setKey("idclase_cuenta", $this->idclase_cuenta->CurrentValue); // Set up key
			} else {
				$this->setKey("idclase_cuenta", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "clase_cuentaview.php")
	...
	function ValidateForm() {
		...
		if (in_array("grupo_cuenta", $DetailTblVar) && $GLOBALS["grupo_cuenta"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// definicion
		$this->definicion->SetDbValueDef($rsnew, $this->definicion->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("grupo_cuenta", $DetailTblVar) && $GLOBALS["grupo_cuenta"]->DetailAdd) {
				$GLOBALS["grupo_cuenta"]->idclase_cuenta->setSessionValue($this->idclase_cuenta->CurrentValue); // Set master key
				if (!isset($GLOBALS["grupo_cuenta_grid"])) $GLOBALS["grupo_cuenta_grid"] = new cgrupo_cuenta_grid(); // Get detail page object
				$AddRow = $GLOBALS["grupo_cuenta_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["grupo_cuenta"]->idclase_cuenta->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("grupo_cuenta", $DetailTblVar)) {
				...
				if ($GLOBALS["grupo_cuenta_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["grupo_cuenta_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["grupo_cuenta_grid"]->CurrentMode = "add";
					$GLOBALS["grupo_cuenta_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($clase_cuenta_add)) $clase_cuenta_add = new cclase_cuenta_add();

// Page init
$clase_cuenta_add->Page_Init();

// Page main
$clase_cuenta_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$clase_cuenta_add->Page_Render();
?>
...
var CurrentForm = fclase_cuentaadd = new ew_Form("fclase_cuentaadd", "add");

// Validate form
fclase_cuentaadd.Validate = function() {
...
fclase_cuentaadd.Form_CustomValidate = 
...
fclase_cuentaadd.ValidateRequired = true;
<?php } else { ?>
fclase_cuentaadd.ValidateRequired = false; 
<?php } ?>
...
<?php $clase_cuenta_add->ShowPageHeader(); ?>
<?php
$clase_cuenta_add->ShowMessage();
?>
<form name="fclase_cuentaadd" id="fclase_cuentaadd" class="<?php echo $clase_cuenta_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($clase_cuenta_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $clase_cuenta_add->Token ?>">
<?php } ?>
...
<?php
	if (in_array("grupo_cuenta", explode(",", $clase_cuenta->getCurrentDetailTable())) && $grupo_cuenta->DetailAdd) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $clase_cuenta_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fclase_cuentaadd.Init();
</script>
<?php
$clase_cuenta_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$clase_cuenta_add->Page_Terminate();
?>

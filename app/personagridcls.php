...
$persona_grid = NULL; // Initialize page object first

class cpersona_grid extends cpersona {
	...
	// Page object name
	var $PageObjName = 'persona_grid';

	// Grid form hidden field names
	var $FormName = 'fpersonagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["persona"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idpersona->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_tipo_persona") && $objForm->HasValue("o_tipo_persona") && $this->tipo_persona->CurrentValue <> $this->tipo_persona->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nombre") && $objForm->HasValue("o_nombre") && $this->nombre->CurrentValue <> $this->nombre->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_apellido") && $objForm->HasValue("o_apellido") && $this->apellido->CurrentValue <> $this->apellido->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idpersona->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->tipo_persona->OldValue = $this->tipo_persona->CurrentValue;
	...
	function LoadFormValues() {
		...
		$this->tipo_persona->setOldValue($objForm->GetValue("o_tipo_persona"));
		...
		$this->apellido->setOldValue($objForm->GetValue("o_apellido"));
		if (!$this->idpersona->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idpersona->CurrentValue = strval($arKeys[0]); // idpersona
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->idpais->getSessionValue() <> "") {
			$rsnew['idpais'] = $this->idpais->getSessionValue();
		}

...
$correlativo_grid = NULL; // Initialize page object first

class ccorrelativo_grid extends ccorrelativo {
	...
	// Page object name
	var $PageObjName = 'correlativo_grid';

	// Grid form hidden field names
	var $FormName = 'fcorrelativogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["correlativo"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idcorrelativo->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idcorrelativo->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->idcorrelativo->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idcorrelativo->CurrentValue = strval($arKeys[0]); // idcorrelativo
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->codigo->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(codigo = '" . ew_AdjustSql($this->codigo->CurrentValue, $this->DBID) . "')";

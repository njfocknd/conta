...
$subcuenta_grid = NULL; // Initialize page object first

class csubcuenta_grid extends csubcuenta {
	...
	// Page object name
	var $PageObjName = 'subcuenta_grid';

	// Grid form hidden field names
	var $FormName = 'fsubcuentagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["subcuenta"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idsubcuenta->CurrentValue = strval($arKeys[0]); // idsubcuenta
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "cuenta_mayor_auxiliar") {
			$this->idcuenta_mayor_auxiliar->Visible = FALSE;
			if ($GLOBALS["cuenta_mayor_auxiliar"]->EventCancelled) $this->EventCancelled = TRUE;
		}

...
class cpais_add extends cpais {
	...
	function ValidateForm() {
		...
		if (in_array("persona", $DetailTblVar) && $GLOBALS["persona"]->DetailAdd) {
			if (!isset($GLOBALS["persona_grid"])) $GLOBALS["persona_grid"] = new cpersona_grid(); // get detail page object
			$GLOBALS["persona_grid"]->ValidateGridForm();
		}
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("persona", $DetailTblVar) && $GLOBALS["persona"]->DetailAdd) {
				$GLOBALS["persona"]->idpais->setSessionValue($this->idpais->CurrentValue); // Set master key
				if (!isset($GLOBALS["persona_grid"])) $GLOBALS["persona_grid"] = new cpersona_grid(); // Get detail page object
				$AddRow = $GLOBALS["persona_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["persona"]->idpais->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("persona", $DetailTblVar)) {
				...
				if ($GLOBALS["persona_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["persona_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["persona_grid"]->CurrentMode = "add";
					$GLOBALS["persona_grid"]->CurrentAction = "gridadd";
...
// Page object
var pais_add = new ew_Page("pais_add");
pais_add.PageID = "add"; // Page ID
var EW_PAGE_ID = pais_add.PageID; // For backward compatibility

// Form object
var fpaisadd = new ew_Form("fpaisadd");
...
?>
<form name="fpaisadd" id="fpaisadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<?php
	if (in_array("persona", explode(",", $pais->getCurrentDetailTable())) && $persona->DetailAdd) {
?>

...
$correlativo_view = NULL; // Initialize page object first

class ccorrelativo_view extends ccorrelativo {
	...
	// Page object name
	var $PageObjName = 'correlativo_view';
	...
	function __construct() {
		...
		if (@$_GET["idcorrelativo"] <> "") {
			$this->RecKey["idcorrelativo"] = $_GET["idcorrelativo"];
			$KeyUrl .= "&amp;idcorrelativo=" . urlencode($this->RecKey["idcorrelativo"]);
		}
	...
	function Page_Init() {
		...
		$this->idcorrelativo->Visible = !$this->IsAdd() && !$this->IsCopy() && !$this->IsGridAdd();
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "correlativolist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "correlativolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->idcorrelativo->HrefValue = "";
			$this->idcorrelativo->TooltipValue = "";
...
// Create page object
if (!isset($correlativo_view)) $correlativo_view = new ccorrelativo_view();

// Page init
$correlativo_view->Page_Init();

// Page main
$correlativo_view->Page_Main();
...
fcorrelativoview.ValidateRequired = true;
<?php } else { ?>
fcorrelativoview.ValidateRequired = false; 
<?php } ?>
...
<?php $correlativo_view->ExportOptions->Render("body") ?>
<?php
	foreach ($correlativo_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $correlativo_view->ShowPageHeader(); ?>
<?php
$correlativo_view->ShowMessage();
?>
<form name="fcorrelativoview" id="fcorrelativoview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($correlativo_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $correlativo_view->Token ?>">
<?php } ?>
...
<script type="text/javascript">
fcorrelativoview.Init();
</script>
<?php
$correlativo_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">

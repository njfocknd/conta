...
class cpais extends cTable {
	...
	function GetDetailUrl() {
		...
		if ($this->getCurrentDetailTable() == "persona") {
			$sDetailUrl = $GLOBALS["persona"]->GetListUrl() . "?showmaster=" . $this->TableVar;
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('idpais', $rs))
				ew_AddFilter($where, ew_QuotedName('idpais') . '=' . ew_QuotedValue($rs['idpais'], $this->idpais->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@idpais@", ew_AdjustSql($this->idpais->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("paisview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("paisview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "paisadd.php?" . $this->UrlParm($parm);
		else
			return "paisadd.php";
	}

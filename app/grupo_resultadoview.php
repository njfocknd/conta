...
$grupo_resultado_view = NULL; // Initialize page object first

class cgrupo_resultado_view extends cgrupo_resultado {
	...
	// Page object name
	var $PageObjName = 'grupo_resultado_view';
	...
	function __construct() {
		...
		if (@$_GET["idgrupo_resultado"] <> "") {
			$this->RecKey["idgrupo_resultado"] = $_GET["idgrupo_resultado"];
			$KeyUrl .= "&amp;idgrupo_resultado=" . urlencode($this->RecKey["idgrupo_resultado"]);
		}
	...
	function Page_Init() {
		...
		$this->idgrupo_resultado->Visible = !$this->IsAdd() && !$this->IsCopy() && !$this->IsGridAdd();
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["idgrupo_resultado"] <> "") {
				...
				$this->RecKey["idgrupo_resultado"] = $this->idgrupo_resultado->QueryStringValue;
			} elseif (@$_POST["idgrupo_resultado"] <> "") {
				$this->idgrupo_resultado->setFormValue($_POST["idgrupo_resultado"]);
				$this->RecKey["idgrupo_resultado"] = $this->idgrupo_resultado->FormValue;
			} else {
				$sReturnUrl = "grupo_resultadolist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "grupo_resultadolist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "grupo_resultadolist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($grupo_resultado_view)) $grupo_resultado_view = new cgrupo_resultado_view();

// Page init
$grupo_resultado_view->Page_Init();

// Page main
$grupo_resultado_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$grupo_resultado_view->Page_Render();
?>
...
var CurrentForm = fgrupo_resultadoview = new ew_Form("fgrupo_resultadoview", "view");

// Form_CustomValidate event
fgrupo_resultadoview.Form_CustomValidate = 
...
fgrupo_resultadoview.ValidateRequired = true;
<?php } else { ?>
fgrupo_resultadoview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fgrupo_resultadoview.Lists["x_idclase_resultado"] = {"LinkField":"x_idclase_resultado","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fgrupo_resultadoview.Lists["x_estado"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fgrupo_resultadoview.Lists["x_estado"].Options = <?php echo json_encode($grupo_resultado->estado->Options()) ?>;
...
<?php $grupo_resultado_view->ExportOptions->Render("body") ?>
<?php
	foreach ($grupo_resultado_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $grupo_resultado_view->ShowPageHeader(); ?>
<?php
$grupo_resultado_view->ShowMessage();
?>
<form name="fgrupo_resultadoview" id="fgrupo_resultadoview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($grupo_resultado_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $grupo_resultado_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="grupo_resultado">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($grupo_resultado->idgrupo_resultado->Visible) { // idgrupo_resultado ?>
	<tr id="r_idgrupo_resultado">
		<td><span id="elh_grupo_resultado_idgrupo_resultado"><?php echo $grupo_resultado->idgrupo_resultado->FldCaption() ?></span></td>
		<td data-name="idgrupo_resultado"<?php echo $grupo_resultado->idgrupo_resultado->CellAttributes() ?>>
<span id="el_grupo_resultado_idgrupo_resultado">
<span<?php echo $grupo_resultado->idgrupo_resultado->ViewAttributes() ?>>
<?php echo $grupo_resultado->idgrupo_resultado->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($grupo_resultado->idclase_resultado->Visible) { // idclase_resultado ?>
	<tr id="r_idclase_resultado">
		<td><span id="elh_grupo_resultado_idclase_resultado"><?php echo $grupo_resultado->idclase_resultado->FldCaption() ?></span></td>
...
<?php echo $grupo_resultado->idclase_resultado->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($grupo_resultado->nombre->Visible) { // nombre ?>
	<tr id="r_nombre">
		<td><span id="elh_grupo_resultado_nombre"><?php echo $grupo_resultado->nombre->FldCaption() ?></span></td>
...
<?php echo $grupo_resultado->nombre->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($grupo_resultado->estado->Visible) { // estado ?>
	<tr id="r_estado">
		<td><span id="elh_grupo_resultado_estado"><?php echo $grupo_resultado->estado->FldCaption() ?></span></td>
		<td data-name="estado"<?php echo $grupo_resultado->estado->CellAttributes() ?>>
<span id="el_grupo_resultado_estado">
<span<?php echo $grupo_resultado->estado->ViewAttributes() ?>>
<?php echo $grupo_resultado->estado->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fgrupo_resultadoview.Init();
</script>
<?php
$grupo_resultado_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$grupo_resultado_view->Page_Terminate();
?>

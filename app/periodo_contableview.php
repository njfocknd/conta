...
class cperiodo_contable_view extends cperiodo_contable {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			} elseif (@$_POST["idperiodo_contable"] <> "") {
				$this->idperiodo_contable->setFormValue($_POST["idperiodo_contable"]);
				$this->RecKey["idperiodo_contable"] = $this->idperiodo_contable->FormValue;
			} else {
	...
	function SetupOtherOptions() {
		...
		$item = &$option->Add("detail_balance_general");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("balance_general", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["balance_general_grid"] && $GLOBALS["balance_general_grid"]->DetailView) {
		...
		if ($GLOBALS["balance_general_grid"] && $GLOBALS["balance_general_grid"]->DetailEdit) {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("balance_general", $DetailTblVar)) {
				...
				if ($GLOBALS["balance_general_grid"]->DetailView) {
					$GLOBALS["balance_general_grid"]->CurrentMode = "view";
...
var CurrentForm = fperiodo_contableview = new ew_Form("fperiodo_contableview", "view");
...
fperiodo_contableview.Lists["x_estado"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fperiodo_contableview.Lists["x_estado"].Options = <?php echo json_encode($periodo_contable->estado->Options()) ?>;
fperiodo_contableview.Lists["x_estatus"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fperiodo_contableview.Lists["x_estatus"].Options = <?php echo json_encode($periodo_contable->estatus->Options()) ?>;
...
<?php if ($periodo_contable->idperiodo_contable->Visible) { // idperiodo_contable ?>
	<tr id="r_idperiodo_contable">
		...
		<td data-name="idperiodo_contable"<?php echo $periodo_contable->idperiodo_contable->CellAttributes() ?>>
<span id="el_periodo_contable_idperiodo_contable">
...
<?php if ($periodo_contable->nombre->Visible) { // nombre ?>
	<tr id="r_nombre">
		<td><span id="elh_periodo_contable_nombre"><?php echo $periodo_contable->nombre->FldCaption() ?></span></td>
...
<?php echo $periodo_contable->nombre->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($periodo_contable->fecha_inicio->Visible) { // fecha_inicio ?>
	<tr id="r_fecha_inicio">
		<td><span id="elh_periodo_contable_fecha_inicio"><?php echo $periodo_contable->fecha_inicio->FldCaption() ?></span></td>
...
<?php echo $periodo_contable->fecha_inicio->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($periodo_contable->fecha_fin->Visible) { // fecha_fin ?>
	<tr id="r_fecha_fin">
		<td><span id="elh_periodo_contable_fecha_fin"><?php echo $periodo_contable->fecha_fin->FldCaption() ?></span></td>
...
<?php echo $periodo_contable->fecha_fin->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($periodo_contable->estado->Visible) { // estado ?>
	<tr id="r_estado">
		...
		<td data-name="estado"<?php echo $periodo_contable->estado->CellAttributes() ?>>
...
<?php if ($periodo_contable->estatus->Visible) { // estatus ?>
	<tr id="r_estatus">
		<td><span id="elh_periodo_contable_estatus"><?php echo $periodo_contable->estatus->FldCaption() ?></span></td>
...
<?php echo $periodo_contable->estatus->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php
	if (in_array("balance_general", explode(",", $periodo_contable->getCurrentDetailTable())) && $balance_general->DetailView) {
?>

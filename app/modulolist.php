...
class cmodulo_list extends cmodulo {
	...
	function __construct() {
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fmodulolistsrch";
	...
	function GetFilterList() {
		...
		// Initialize
		$sFilterList = "";
		$sFilterList = ew_Concat($sFilterList, $this->idmodulo->AdvancedSearch->ToJSON(), ","); // Field idmodulo
	...
	function RestoreFilterList() {
		...
		// Field idmodulo
		$this->idmodulo->AdvancedSearch->SearchValue = @$filter["x_idmodulo"];
		$this->idmodulo->AdvancedSearch->SearchOperator = @$filter["z_idmodulo"];
		$this->idmodulo->AdvancedSearch->SearchCondition = @$filter["v_idmodulo"];
		$this->idmodulo->AdvancedSearch->SearchValue2 = @$filter["y_idmodulo"];
		$this->idmodulo->AdvancedSearch->SearchOperator2 = @$filter["w_idmodulo"];
		$this->idmodulo->AdvancedSearch->Save();
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->idmodulo->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event);'>";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fmodulolistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fmodulolistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fmodulolist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
...
var CurrentForm = fmodulolist = new ew_Form("fmodulolist", "list");
...
var CurrentSearchForm = fmodulolistsrch = new ew_Form("fmodulolistsrch");
</script>
<script type="text/javascript">
...
<?php } ?>
<?php if ($modulo_list->FilterOptions->Visible()) { ?>
<?php $modulo_list->FilterOptions->Render("body") ?>
<?php } ?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php
	$bSelectLimit = $modulo_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($modulo_list->TotalRecs <= 0)
			$modulo_list->TotalRecs = $modulo->SelectRecordCount();
	} else {
		if (!$modulo_list->Recordset && ($modulo_list->Recordset = $modulo_list->LoadRecordset()))
...
// Header row
$modulo_list->RowType = EW_ROWTYPE_HEADER;
...
<span id="el<?php echo $modulo_list->RowCnt ?>_modulo_nombre" class="modulo_nombre">
...
fmodulolistsrch.FilterList = <?php echo $modulo_list->GetFilterList() ?>;
fmodulolist.Init();
</script>
<?php

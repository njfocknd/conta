    ...
    function list_orders($query)
    {
		...
		if($link)
		{
				...
				if ($db_selected)
				{
						...
						{						
							// while ($row = mysql_fetch_assoc($result)) {
								// echo "<TR><TD><INPUT TYPE='CHECKBOX'></TD>"."<TD>123</TD>"."<TD>".$row["cliente_id"]."</TD>"."<TD>".$row["lastname"]."</TD>"."<TD>".$row["firstname"]."</TD>"."<TD>".$row["idorder"]."</TD>"."<TD>".$row["edition_date"]."</TD>"."<TD>".$row["total_items"]."</TD>"."<TD>".$row["total_price"]."</TD>"."<TD>".$row["name"]."</TD>"."<TD>".$row["username"]."</TD></TR>";
                            // }
						}
						 else {
								$result = null;
								//echo "Lo sentimos no hay informacion con los filtros seleccionados.";
						}
				}  else 
				{
						$result = null;
						echo "No se pudo establecer la conexion con la base de datos, llame a su administrador.";//die('Could not connect:2 ' . mysql_error());
				}
				// Closing connection
				mysql_close($link);
		}
